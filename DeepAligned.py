...
class ModelManager:
    ...
    def __init__(self, args, data, pretrained_model=None):
        ...
        if pretrained_model is None:
            pretrained_model = BertForModel.from_pretrained(args.bert_model, cache_dir = "", n_labels = data.n_known_cls)
        ...
        self.model = BertForModel.from_pretrained(args.bert_model, cache_dir = "", n_labels = self.num_labels)
        ...
        self.num_train_optimization_steps = int(math.ceil(num_train_examples / args.train_batch_size)) * args.num_train_epochs
    ...
    def predict_k(self, args, data):
        ...
        km = Kmeans(feats.shape[1], self.num_labels, nredo=args.nredo, niter=args.num_kmean_iter, max_points_per_centroid=args.max_points_per_centroid, verbose=True, gpu=True)
    ...
    def get_optimizer(self, args):
        ...
        #                  lr=args.lr,
        #                  correct_bias=False)
        # num_warmup_steps = self.num_train_optimization_steps * args.warmup_proportion
        # get_linear_schedule_with_warmup(optimizer, num_warmup_steps=num_warmup_steps, num_training_steps=self.num_train_optimization_steps)
    ...
    def alignment(self, km, feats, args):
        _, I = km.index.search(feats, 1)
        labels = I.flatten()
        if self.centroids is not None:

            old_centroids = self.centroids.cpu().numpy()
            new_centroids = km.centroids
            ...
            pseudo_labels = np.array([pseudo2label[label] for label in labels])

        else:
            self.centroids = torch.tensor(km.centroids).to(self.device)
    ...
    def update_pseudo_labels(self, pseudo_labels, args, data):
        train_data = TensorDataset(data.semi_input_ids, data.semi_input_mask, pseudo_labels)
    ...
    def train(self, args, data): 
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):  
            ...
            pseudo_labels = self.alignment(km, feats, args)
            ...
            for batch in tqdm(train_dataloader, desc="Pseudo-Training"):
                ...
                loss = self.model(input_ids, input_mask, label_ids, mode='train')
    ...
    def save_model(self, args):
        ...
        model_file = os.path.join(args.final_model_dir, WEIGHTS_NAME)
        model_config_file = os.path.join(args.final_model_dir, CONFIG_NAME)
