...
class Data:
    ...
    def __init__(self, args):
        set_seed(args.seed)
        max_seq_lengths = {'clinc':30, 'stackoverflow':45,'banking':55, 'vcb':16}
        args.max_seq_length = max_seq_lengths[args.dataset]

        self.processor = DatasetProcessor()
        self.data_dir = os.path.join(args.data_dir, args.dataset)
        self.all_label_list = self.processor.get_labels(self.data_dir)
        self.known_label_list = self.processor.get_known_labels(self.data_dir)
        self.n_known_cls = len(self.known_label_list)

        self.num_labels = int(len(self.all_label_list) * args.cluster_num_factor)

    def load(self, args):
        self.train_labeled_examples, self.train_unlabeled_examples = self.get_examples(self.processor, args, 'train')
        print('num_labeled_samples',len(self.train_labeled_examples))
        print('num_unlabeled_samples',len(self.train_unlabeled_examples))
        self.eval_examples = self.get_examples(self.processor, args, 'eval')
        self.test_examples = self.get_examples(self.processor, args, 'test')
        self.train_labeled_dataloader = self.get_loader(self.train_labeled_examples, args, 'train')

        self.semi_input_ids, self.semi_input_mask, self.semi_label_ids = self.get_semi(self.train_labeled_examples, self.train_unlabeled_examples, args)
        self.train_semi_dataloader = self.get_semi_loader(self.semi_input_ids, self.semi_input_mask, self.semi_label_ids, args)
    ...
    def get_type(self, args, mode='full'):
        examples = self.get_examples(self.processor, args, mode)
        return self.get_loader(examples, args, mode)
    ...
    def get_examples(self, processor, args, mode = 'train'):
        ...
        if mode == 'train':
            ...
            train_num_labels = defaultdict(int)
            for label in train_labels:
                train_num_labels[label] += 1
            ...
            # for label in self.known_label_list:
            #     num = train_num_labels[label] if train_num_labels[label] < args.num_unlabeled else args.num_unlabeled
            #     pos = list(np.where(train_labels == label)[0])
            #     train_unlabeled_ids.extend(random.sample(pos, num))
    ...
    def get_semi(self, labeled_examples, unlabeled_examples, args):
        ...
        return semi_input_ids, semi_input_mask, semi_label_ids
    ...
    def get_loader(self, examples, args, mode = 'train'):
        ...
        elif mode == 'eval' or mode == 'test' or mode == 'full':
    ...
    def get_examples(self, data_dir, mode):
        if mode == 'train':
            return self._create_examples(
                self._read_tsv(os.path.join(data_dir, "train_without_context.tsv")), "train")
        elif mode == 'eval':
            return self._create_examples(
                self._read_tsv(os.path.join(data_dir, "dev_without_context.tsv")), "train")
        elif mode == 'test':
            return self._create_examples(
                self._read_tsv(os.path.join(data_dir, "test_without_context.tsv")), "test")
        elif mode == 'full':
            return self._create_examples(
                self._read_tsv(os.path.join(data_dir, "full_without_context.tsv")), None)

    def get_labels(self, data_dir):
        """See base class."""
        import pandas as pd
        test = pd.read_csv(os.path.join(data_dir, "labels_without_context.txt"), header=None)
        labels = np.unique(np.array(test[0]))

        return labels

    def get_known_labels(self, data_dir):
        """See base class."""
        import pandas as pd
        test = pd.read_csv(os.path.join(data_dir, "known_labels_without_context.txt"), header=None)
        labels = np.unique(np.array(test[0]))
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if len(input_ids) < max_seq_length:
            ...
            pad_token = [1] * (max_seq_length - len(input_ids))
            input_ids += pad_token
            input_mask += padding
            # segment_ids += padding
        else :
